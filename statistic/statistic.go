...
func (tsds Tsds) PutArray2Data(arr numgo.Array) error {
	if len(tsds) != len(arr) {
		return fmt.Errorf("PutArray2Data: Destination array and source array must be the same length.[目的数组和源数组长度必须相同]")
	}
...
***********************************************************
	...
	输入: diff float64:阈值
	输出:
	说明: 一个数据与其前一个数据的差值为负(或为正)，于其后一个数的插值为正(或为负)
		且两个插值的绝对值均大于阈值，则判定其为奇异点。用其前一个数与后一个数的评价值替换之
	编辑: wangjp
	时间: 2023年11月30日

***********************************************************
*/
func (tsds Tsds) DiffFilter(diff float64) {
	dlen := len(tsds)
	for i, tsd := range tsds {
		if i == 0 || i == dlen-1 {
			continue
		}
		diff0 := tsd.Value - tsds[i-1].Value
		diff1 := tsds[i+1].Value - tsd.Value
		if (diff0 < 0 && diff1 > 0) || (diff0 > 0 && diff1 < 0) { //两个差的符号相反
			if math.Abs(diff0) > diff && math.Abs(diff1) > diff { //且两个差的绝对值都大于阈值
				tsd.Value = (tsds[i-1].Value + tsds[i+1].Value) / 2.0 //用前后两个值的均值替代之
				tsds[i] = tsd
			}
		}
	}
...
func (tsds Tsds) ButterFilter(N int, Wn interface{}, btype string, fs ...float64) error {
	arr := tsds.GetDataArray()
	arr, err := filter.ButterFilter(arr, N, Wn, btype, fs...)
	if err != nil {
		return err
	}
